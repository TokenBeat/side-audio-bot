...
export function resolveDisabledFrontendTools(env = process.env) {
  return [
    ...(!featureEnabled(env.SIDE_AUDIO_SCHEDULE_TOOL_ENABLED)
      ? ['schedule_reminder'] : []),
    ...(!featureEnabled(env.SIDE_AUDIO_WEB_TOOLS_ENABLED)
      ? ['web_search', 'fetch_url'] : []),
    ...(!featureEnabled(env.SIDE_AUDIO_KNOWLEDGE_TOOL_ENABLED)
      ? ['knowledge'] : []),
    ...(!featureEnabled(env.SIDE_AUDIO_NOTES_TOOL_ENABLED)
      ? ['notes'] : []),
    ...(!featureEnabled(env.SIDE_AUDIO_RECALL_TOOL_ENABLED)
      ? ['recall'] : []),
  ]
}
...
export const config = {
  ...
  webSkinsDirectory: process.env.SIDE_AUDIO_WEB_SKINS_DIR
    ? resolve(process.env.SIDE_AUDIO_WEB_SKINS_DIR)
    : '',
  ...
  tailnet: ['1', 'true', 'yes', 'on'].includes(
    String(process.env.SIDE_AUDIO_GATEWAY_TAILNET || '').trim().toLowerCase(),
  ),
  lan: ['1', 'true', 'yes', 'on'].includes(
    String(process.env.SIDE_AUDIO_GATEWAY_LAN || '').trim().toLowerCase(),
  ),
  gatewayLanHost: String(
    process.env.SIDE_AUDIO_GATEWAY_LAN_HOST || '',
  ).trim(),
  ...
  memoryProvider: normalizeMemoryProviderSelection(
    process.env.SIDE_AUDIO_MEMORY_PROVIDER,
  ),
  ...
  gatewayAccessToken: String(
    process.env.SIDE_AUDIO_GATEWAY_ACCESS_TOKEN
    || process.env.SIDE_AUDIO_BOT_ACCESS_TOKEN
    || '',
  ).trim(),
  gatewayAccessKeys: String(
    process.env.SIDE_AUDIO_BOT_ACCESS_KEYS || '',
  ).trim(),
  ...
  sessionDigestEnabled: String(
    process.env.SIDE_AUDIO_SESSION_DIGEST || 'off',
  ).toLowerCase() === 'on',
  ...
  domainLibraryEnabled: String(
    process.env.SIDE_AUDIO_DOMAIN_LIBRARY || 'off',
  ).toLowerCase() === 'on',
