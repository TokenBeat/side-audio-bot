export function webRtcOptions(env = process.env) {
  if (!['1', 'true'].includes(env.SIDEAUDIO_WEBRTC_ENABLED)) return null
  const iceServers = JSON.parse(env.SIDEAUDIO_WEBRTC_ICE_SERVERS || '[]')
  ...
  const iceTransportPolicy = env.SIDEAUDIO_WEBRTC_ICE_TRANSPORT_POLICY || 'all'
