# Side Audio Bot Memcode 集成示例

[English](README.md) | 中文

这个可选示例通过 side-audio-bot 的 `MemoryProvider` v2 接口接入
...
| 组件 | 职责 |
|---|---|
| side-audio-bot Gateway | 语音对话、记忆工具和 Provider 生命周期。 |
...
在 [Memcode 控制台](https://app.memcode.in/dashboard?section=api-keys&integration=side-audio-bot)
创建密钥，集成选择 **Side Audio Bot**。编辑 `.env.local`，填写密钥和语音前台配置。
使用默认前台时：
...
默认 owner 为 `user_personal`，可通过 `SIDE_AUDIO_BOT_PERSONAL_OWNER_ID` 调整。
远端身份由 Memcode 凭据决定；适配器不发送 `user_id` 或归因覆盖字段。

按上述方式启动时，快照保存在
`examples/memcode/.side-audio/runtime/memory/memcode/snapshot.json`。
启动器根据 `SIDEAUDIO_CONFIG_DIR` 确定此路径，相对路径以启动目录为基准。文件包含私有记忆和待处理修改，
...
- **其他宿主接入：** `sessionObservation: false` 本身不会关闭框架的自动学习。
  不使用本启动器时，如只允许显式写入，宿主需自行关闭 `SIDE_AUDIO_MEMORY_AUTO`
  和 `SIDE_AUDIO_PREFERENCE_LEARNING`。此适配器不转发原始音频。
...
- [Vivek Gupta](https://github.com/vivekgupta-memcode)：在
  [PR #488](https://github.com/TokenBeat/side-audio-bot/pull/488) 中贡献 Memcode
