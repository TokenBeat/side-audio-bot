...
- care：床位平面图主视觉、护理员手持端、交班文书语音录入（健康趋势图表化已完成：家属端近 7 天四指标趋势图）
- home：家属留言录音播放（真音频）、Capacitor 真机（周报图表已完成：家属端"这一周"卡）
- hub：真实设备协议接入（Matter/米家 bridge）（天气与能耗面板、传感器历史曲线已完成：环境与能耗卡）
...
- 家属端近 7 天健康趋势图：四指标切换、参考线、异常红点（本地提交，见顶部冲刺说明）
...
⬜ 待办：电话外呼（SIP，问安打到老人手机）、声学异常感知、真机 App（Capacitor）。（周报已完成：家属端 weekly 卡）
