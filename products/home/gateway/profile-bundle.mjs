...
export function createHomeFrontendMcpConfiguration({
  frontendMcpUrl = process.env.CARE_FRONTEND_MCP_URL,
} = {}) {
  if (!frontendMcpUrl) throw new Error('Home frontend MCP URL is required')
  return {
    version: 1,
    servers: {
      home: {
...
export function writeHomeFrontendProfileBundle({
  root = process.env.QWAUDIO_CONFIG_DIR,
  frontendMcpUrl = process.env.CARE_FRONTEND_MCP_URL,
} = {}) {
  if (!root) throw new Error('Home frontend profile bundle root is required')
  ...
  writeFileSync(
    mcpPath,
    `${JSON.stringify(createHomeFrontendMcpConfiguration({ frontendMcpUrl }), null, 2)}\n`,
  )
  writeFileSync(
    profilePath,
    `${JSON.stringify({
      version: 1,
      name: 'wanqing-home',
      description: '晚晴·居家老人端的前台语音伙伴：温和、笃定、叫得应人。',
