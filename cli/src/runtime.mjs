...
export function assertMemoryGatewayCompatibility(health, env = process.env) {
  const configured = String(env.SIDE_AUDIO_MEMORY_PROVIDER || '').trim()
...
export async function ensureRuntime(options, {
  ...
  if (options.webrtc) env.SIDEAUDIO_WEBRTC_ENABLED = '1'
  ...
  try {
    if (!health) {
      ...
      if (['1', 'true'].includes(env.SIDEAUDIO_WEBRTC_ENABLED)) requireWebRtc()
