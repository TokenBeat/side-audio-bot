...
test('a consumer with only the declared dependencies can run the CLI and Gateway', { skip: !enabled }, async t => {
  ...
  const sdk = spawnSync(process.execPath, [
    ...
    '--eval',
    [
      "import { BACKEND_ADAPTER_SDK_VERSION, defineBackendAdapter } from 'side-audio-bot/backend-adapter-sdk'",
...
async function main() {
  ...
  const clientDir = mkdtempSync(join(tmpdir(), 'sideaudio-client-'))
