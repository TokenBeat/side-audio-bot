...
test('importing TaskManager does not initialize user configuration or durable state', t => {
  const directory = mkdtempSync(join(tmpdir(), 'sideaudio-task-import-'))
  ...
  const child = spawnSync(process.execPath, ['--input-type=module', '--eval',
    ...
    env: { ...process.env, SIDEAUDIO_CONFIG_DIR: configDirectory }, encoding: 'utf8',
  })
...
test('the CLI doctor entry point does not create user configuration or logs', t => {
  const directory = mkdtempSync(join(tmpdir(), 'sideaudio-doctor-import-'))
  t.after(() => rmSync(directory, { recursive: true, force: true }))
  const configDirectory = join(directory, 'not-created')
  const child = spawnSync(process.execPath, [resolve(import.meta.dirname, '../cli/bin/sideaudio.mjs'),
    'doctor', '--url', 'http://127.0.0.1:1', '--json',
  ], {
    env: { ...process.env, SIDEAUDIO_CONFIG_DIR: configDirectory, AGENT_PROTOCOL: 'none', SIDE_AUDIO_LOG_FILE: '1' },
    encoding: 'utf8', timeout: 10_000,
  })
  assert.equal(child.status, 1, child.stderr)
  assert.equal(JSON.parse(child.stdout).schema, 'sideaudio.diagnostics/v1')
  assert.equal(existsSync(configDirectory), false)
})
