...
当从上游 `qwen-audio-agent` 合并代码时，注意以下冲突点：
- 环境变量名：上游使用 `QWEN_AUDIO_AGENT_*`，本地使用 `SIDE_AUDIO_BOT_*`；Gateway/Log/Memory/WakeWord 使用 `SIDE_AUDIO_*`
- 数据目录：上游使用 `~/.config/qwaudio`，本地使用 `~/.config/sideaudio`
- 密钥名：上游使用 `QWEN_AUDIO_AGENT_AUTH_SECRET`，本地使用 `SIDE_AUDIO_BOT_AUTH_SECRET`
- CLI 入口：上游使用 `cli/bin/qwenaudio.mjs`，本地使用 `cli/bin/sideaudio.mjs`

建议合并时优先保留本地修改，或根据上游变化手动同步文本替换。
