...
产品根目录默认为 `~/.config/sideaudio`。网关和 TUI 按目录归属管理各自的数据，
不要求为每个进程建立一个独立根目录：
...
| 环境变量 | 用途 | 默认值 |
| --- | --- | --- |
| `SIDEAUDIO_CONFIG_DIR` | 产品根目录；启动前设置 | `$XDG_CONFIG_HOME/sideaudio`，未设置 XDG 时为 `~/.config/sideaudio` |
| `SIDEAUDIO_DATA_DIR` | 共享用户数据 | `<config-dir>/data` |
| `SIDEAUDIO_STATE_DIR` | 当前 Gateway 的持久状态 | CLI 为 `<config-dir>/state`；桌面代管为 `<config-dir>/state/desktop` |
| `SIDEAUDIO_CACHE_DIR` | 可重建缓存 | `<config-dir>/cache` |
| `SIDEAUDIO_WORKSPACE` | 所有后台的默认工作区 | `<data-dir>/workspace` |
...
- Linux：`$XDG_CONFIG_HOME/Side Audio Bot`，默认 `~/.config/Side Audio Bot`
...
TUI 的连接配置与凭据保存在 `<config-dir>/tui/`，默认 `~/.config/sideaudio/tui/`；
实例锁和诊断日志也放在这里。CLI 的 `connect`、`disconnect`、`tui` 命令管理这些文件，
Gateway 不读取或写入它们。

修改 `SIDEAUDIO_CONFIG_DIR` 时，TUI 目录随产品根目录改变；单独修改
`SIDEAUDIO_DATA_DIR`、`SIDEAUDIO_STATE_DIR` 或 `SIDEAUDIO_CACHE_DIR` 不影响 TUI。
如需独立指定位置，可在启动前设置 `SIDEAUDIO_TUI_DIR`。桌面应用目录不跟随这些变量。
