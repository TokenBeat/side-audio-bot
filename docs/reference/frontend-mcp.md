...
Save one of these JSON configurations at the configured path, supply real server/tool names and credentials, then [restart the Gateway](../operations/gateway.md#applying-configuration-changes). Test a simple call before enabling more tools. Use `sideaudio doctor` and Gateway logs for connection failures.
